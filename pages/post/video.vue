  ...
  <div>
    ...
    <div class="turn-time-box">
      ...
      <div class="v-bottom" style="margin-top:57px;">
        <div class="turn-time">
          <turnList listName="newest" :chooseData="nowSelArr" :keywords="searchKeyWords" :isNew="true" ref="newlist" :emptyCheck='true'>
          ...
          <div class="m-have-pages">
            <turnList listName="create" style="margin-top:34px;" :keywords="searchKeyWords" :chooseData="nowSelArr" :showPage="true">
              <turnTitle slot="title" :tab-arr="titleTabArr" class="t1">
                <div class="sear-box" slot="btn">
                  <input type="text" v-model="keywords" placeholder="搜索视频" @keyup.enter="searchSubmit">
        ...
        <turnAside type="2" param="deal,play,comm" title="热门上刊视频"></turnAside>
      </div>
      <!-- 列表 -->
    </div>
  </div>
...
export default {
  ...
  head(){
    return {
      title: `上刊视频列表 - ${this.indexData.create.current_page} - 凌天众媒网`,
      meta: [{
        hid: "description",
        name: "description",
        content: "上刊视频列表，实时上刊，最新广告上刊，上刊大厅，上刊图片，上刊视频，热门上刊，找上刊资源上凌天众媒网（zhongmei66.com）"
      },{
        hid: "keywords",
        name: "keywords",
        content: "上刊视频列表，最新最全广告上刊，最新最全媒体上刊，上刊大厅，查找投放广告视频就上凌天众媒网"
      }]
    }
  },
  provide () {
  ...
  async asyncData (app) {
    ...
    const dataRes = await getTurnTimeList(app, 2).catch(e => {
      if (e && e.data) {
