      ...
      <div class="user_cen_right">
        <div class="tradeNews">
          <div class="tradeNews_top">
            <span>
              有
              <p>{{countNum}}</p>条交易通知未读
            </span>
            <span v-if="countNum==0" style="color:#f3262d;"></span>
            <span class="read" @click="readAll" v-else>
              <span class="iconfont icon-xiansuo-gou"></span>全部设为已读
            </span>
          </div>
          <div class="tradeNews_cont" v-if="coachExplainList !=null">
            <ul v-if="coachExplainList.length >=1">
              <li v-for="(item , index) in coachExplainList" :key="index" class="tradeNews_li">
                ...
                <div class="tradeNews_left" @click="updata(item , item.status)">
                  ...
                  <a
                    ...
                    class="coach-explain-wrap tradeNews_left_1"
                  ...
                  <a
                    class="btn-pick-up"
                    v-show="item.content.length >= 187"
          ...
          <div class="block" id="block" style="margin: 30px 40px;" v-if="flag">
            <el-pagination
  ...
  filters: {
    explainLen: function(item) {
      ...
      if (item.isExpand) {
        ...
        // 当下收起状态
        return item.content.substr(0, 187); //字符串截取100个字
      }
    }
  }
};
...
@import "tradeNewScode.less";
</style>
<style lang="less">
@import "tradeNews.less";
</style>
<style lang="less">
@import "../index.less";
</style>
