  ...
  >
    ...
    <div class="service_details">
      ...
      <div class="service_Nav">
        <div class="serviced">
          <ul class="nav_classify" v-if="this.$route.params.id || this.$route.query.cate_code">
            <li>
              <div class="nav_one">
                <p>{{SubmenuList.cate_name}}</p>
              </div>
              <svg class="icon" aria-hidden="true">
                <use xlink:href="#icon-xiangyoudianji" />
              </svg>
            </li>
            <li>
              <div class="chang">
              ...
              <div class="Submenu" v-if="SubmenuList.children">
                <div
                  v-for="(item , index) in SubmenuList.children"
                  :key="index"
                  @click="SubmenClick1(item , index)"
                  :class="{SubmenClick1:SubIndex == index}"
                >{{item.cate_name}}</div>
              </div>
              <div class="Submenu noData_lm" v-else>暂无数据</div>
              <svg class="icon" aria-hidden="true">
                <use xlink:href="#icon-xiangyoudianji" />
              </svg>
            </li>
            <li>
              ...
              <div
                class="Submenu"
                v-if="Subchildren.children !=undefined && Subchildren.children.length>0 "
              >
                <div
                  v-for="(item , index) in Subchildren.children"
                  :key="index"
                  @click="SubmenClick2(item , index)"
                  :class="{SubmenClick2:SubIndex2 == index}"
                >{{item.cate_name}}</div>
              </div>
              <div class="Submenu noData_lm" v-else>暂无数据</div>
            </li>
            <svg class="icon ad-arrow" aria-hidden="true">
              <use xlink:href="#icon-xiangyoudianji" />
            </svg>
          </ul>
          <div class="nav_brand" v-else-if="this.$route.query.brand_id">
            当前结果
            <i class="crumbs-arrow">></i>
            <p class="colo">"{{SubmenuList.brand_name}}"</p>
          </div>

          <div class="nav_brand" v-else-if="this.$route.query.keywords">
            当前结果
            <i class="crumbs-arrow">></i>
            <p class="colo">"{{this.$route.query.keywords}}"</p>
          </div>
          <div class="NameList">
          ...
          <div class="shopLis_right_cont">
            <div class="shopLi_ul" v-if="shopLisRightCont.length >0">
              ...
              >
                ...
                <nuxt-link
                  :to="`/service/${Cont.id}?id=${Cont.store_id}`"
                  target="_blank" class="status" v-else>
                  <div class="oldText" style="text-indent:0">{{Cont.name}}</div>
                </nuxt-link>
                <div class="location">
                  <div>
                    <nuxt-link
                ...
                <div class="lzGrade">
                  ...
                  <div class="marginsum" v-if="Cont.marginsum">
                    ...
                    <p>{{parseInt(Cont.marginsum)}}</p>
                    <div>
                      已缴纳诚信保证金
                      <i>{{parseInt(Cont.marginsum)}}</i>元
                    </div>
                  </div>
                </div>
...
export default {
  ...
  head() {
    ...
    var keyWord = "";
    var description = "";
    var cate_name = this.SubmenuList.cate_name;
    if (this.T_rec !== "" && this.S_rec === "") {
      title = this.T_rec + " " + "-" + " " + cate_name + " " + "-" + " ";
      keyWord = this.T_rec + " " + "," + " " + cate_name;
      description =
        this.T_rec + "频道 " + "," + " " + cate_name + " " + "," + " ";
    } else if (this.T_rec !== "" && this.S_rec !== "") {
      title =
        this.S_rec +
        " " +
        "-" +
        ...
        " ";
      keyWord =
        this.S_rec + " " + "," + " " + this.T_rec + " " + "," + " " + cate_name;
      description =
        this.S_rec +
        "频道 " +
        "," +
      ...
      meta: [
        {
          hid: "keywords",
          name: "keywords",
          content: `凌天众媒网${keyWord || ""}`
        },
        {
          hid: "description",
          name: "description",
          content: `凌天众媒网（zhongmei66.com）${description ||
            ""}一站式全媒界大型媒体电商平台`
        }
      ]
    };
  ...
  async asyncData(app) {
    const { id, brand_id } = app.params;
    const { keywords } = app.query;
    var page = app.query.page || 1;
    let params = {
      cate_code: id || "",
      page: page,
      brand_code: app.query.brand_code || "", // 品牌id，品牌筛选使用
    ...
    var data = {
      cate_code: id || "",
      brand_code: app.query.brand_code || ""
    };
    var dataList = {
      cate_code: id || ""
    };
    ...
    let map = new Map();
    if (Flie.brandList.data.length > 0) {
      var letData = Flie.brandList.data; //所有数据集合
      letData.forEach((item, idnex) => {
        letterList.push(item.first_letter);
      });
      letterList = Array.from(new Set(letterList)); //得到字母数组
      // 获取字母所对应的所有数据 key为字母，value为对应的数据集合
      letterList.forEach((l_item, l_index) => {
        let DList = [];
        letData.forEach((item, index) => {
          if (l_item == item.first_letter) {
            DList.push(item);
          }
          map.set(l_item, DList);
        });
      });
      for (var i = 0; i < datal.data.length; i++) {
        datal.data[i].marginsum = Math.ceil(datal.data[i].marginsum);
        datal.data[i] = {
          ...datal.data[i],
          flag: false
        };
      }
      if (Flie.CatInfo.children) {
        var T_reclassify = "全部";
        var T_rec = "";
        var title = "";
        var keyWord = "";
        ...
        var S_rec = "";
        var S_reclassify = "全部";
        for (var i = 0; i < Flie.CatInfo.children.length; i++) {
          var a = Flie.CatInfo.children[i];
          if (Flie.CatInfo.current.cate2) {
            if (Flie.CatInfo.current.cate2 == a.id) {
              T_reclassify = a.cate_name;
              T_rec = a.cate_name;
              Subchildren = a;
              for (var j = 0; j < Subchildren.children.length; j++) {
                var c = Subchildren.children[j];
                if (Flie.CatInfo.current.cate3) {
                  if (Flie.CatInfo.current.cate3 == c.id) {
                    S_reclassify = c.cate_name;
                    S_rec = c.cate_name;
                  }
                }
              }
    ...
    return {
      ...
      navList: Flie.brandList,
      navListImg: Flie.brandList.data,
      SubmenuList: Flie.CatInfo,
      Subchildren: Subchildren || Flie.CatInfo.children[0],
      ...
      letData,
      map: map,
      brandName: Flie.brandList.data,
      T_reclassify: T_reclassify || "全部",
      T_rec: T_rec || "",
      title,
      keyWord: keyWord,
      description: description,
      S_rec: S_rec || "",
      S_reclassify: S_reclassify || "全部",
  ...
  methods: {
    ...
    dataEventTrack({
      ...
      kv: {
        router: `/category/${this.$route.params.id}?page=${this.$route.query.page}`,
    ...
    async init() {
      if (
        ...
        this.cate_code = this.$route.params.id;
      } else {
        this.cate_code = "";
      }
      if (this.item != "") {
      ...
      if (this.$route.query.brand_code) {
        this.brandId = this.$route.query.brand_code;
      } else {
      ...
      var params = {
        ...
        page: this.page,
        brand_id: this.brandId, // 品牌id，品牌筛选使用
    ...
    async filtrate() {
      ...
      if (data.brandList != undefined && data.brandList != "") {
        ...
        this.Subchildren = this.SubmenuList.children[0];
        this.current = this.SubmenuList.current;
        //拿到三级数据
        this.SubmenCurrent();
        let letterList = []; //存所有字母
        let map = new Map();
        if (this.navList.data != undefined && this.navList.data.length > 0) {
          let letData = this.navList.data; //所有数据集合
          letData.forEach((item, idnex) => {
            letterList.push(item.first_letter);
          });
          letterList = Array.from(new Set(letterList)); //得到字母数组
          ...
          this.letData = letData;
          this.map = map;
          if (this.letterIndex > -1) {
            this.brandName = map.get(letterList[this.letterIndex]); //获取字母索引对应的列表集合
          } else {
    ...
    // 点击三级分类分类
    SubmenCurrent() {
      if (this.SubmenuList.children) {
        for (var i = 0; i < this.SubmenuList.children.length; i++) {
          var a = this.SubmenuList.children[i];
          if (this.current.cate2) {
            if (this.current.cate2 == a.id) {
              this.T_reclassify = a.cate_name;
              this.T_rec = a.cate_name;
              this.Subchildren = a;
              for (var j = 0; j < this.Subchildren.children.length; j++) {
                var c = this.Subchildren.children[j];
                if (this.current.cate3) {
                  if (this.current.cate3 == c.id) {
                    this.S_reclassify = c.cate_name;
                    this.S_rec = c.cate_name;
                  }
                }
              }
            }
