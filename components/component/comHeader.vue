      ...
      <div class="login_header" :style="'width:'+headerWid+'px'">
        ...
        <ul class="login_header_right">
          ...
          <li>
            ...
            <p v-if="Token  || !isToken">({{countNumS+countNum}})</p>
            <p v-else>0</p>
            <svg class="icon" aria-hidden="true">
              <use xlink:href="#icon-xiangxia" class="ccccc" />
            </svg>
            <div class="xl_nav message_text">
              <span @click="loginNONE('/userCenter/tradeNews/tradeNews')">
                交易消息 (
                <span v-if="Token  || !isToken">{{countNum}}</span>
                <span v-else>0</span>)
              </span>
              <span @click="loginNONE('/userCenter/systemNews/systemNews')">
                系统消息 (
                <span v-if="Token  || !isToken">{{countNumS}}</span>
                <span v-else>0</span>)
              </span>
            </div>
          </li>
      ...
      <div class="header_center" id="boxFixed" :class="{'is_fixed' : isFixed}">
        <div class="header_center_logo" :style="'width:'+headerWid+'px'">
          ...
          <div class="header_ser">
            <div class="ser_left">
              <div style="margin-top: 29px;" @mouseleave="seaOut()">
                <el-input
                  placeholder="请输入内容"
                  autocomplete="off"
                  v-model="searchInp"
...
export default {
  ...
  mounted() {
    console.log('searchData',JSON.parse(JSON.stringify(this.searchData)));
    ...
    if (userInfo) {
      ...
      if (this.Token && userInfo) {
        this.Token && this.setUser(this.Token);
      }
    }
